...
const useAIChatStreamHandler = () => {
  ...
  const handleStreamResponse = useCallback(
    async (input: string, images?: string[]) => {
      ...
      // Build message content as JSON with optional images
      const messageData = {
        message: input || '',
        images: images && images.length > 0 ? images : undefined,
      }

      // Build FormData payload for AgentOS endpoint
      // Send the entire message data as a JSON string in the 'message' field
      ...
      try {
        ...
        // Build URL with stream and session_id as query parameters (AgentOS framework params)
        const params = new URLSearchParams({
          stream: 'true',
          session_id: sessionId ?? '',
        })
        const urlWithParams = `${RunUrl}?${params.toString()}`
            ...
            if (
              ...
              if (
                ...
                const sessionData = {
                  ...
                  session_name: input || 'New Session',
                ...
                const newMessages = prevMessages.map((message, index) => {
                  if (
                    ...
                    if (typeof chunk.content === 'string') {
                      ...
                      'response' in chunk.content
                    ) {
                      // Handle RecipeResponse: extract the response field
                      updatedContent = (chunk.content as any).response || ''
                    } else {
                      try {
